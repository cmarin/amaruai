...
export function MultiAiChat() {
  ...
  useEffect(() => {
    if (!dataLoading && !error && chatModels.length > 0) {
      ...
      // Initialize chatbots with all available models
      ...
      // Set the first chatbot as active by default
      setActiveChatbots([initialChatbots[0].id])

      // Set layout mode to single
  ...
  const handleSend = async () => {
    if (input.trim()) {
      ...
      // Update chatbots with the user message
      ...
      // Send message to each active chatbot
      for (const botId of activeChatbots) {
        ...
        if (bot) {
          ...
          try {
            // Find the persona object based on the bot's persona role
            ...
            // Only add persona_id if a persona is selected
            if (personaObject) {
              ...
              const parser = createParser((event: ParsedEvent | ReconnectInterval) => {
                if (event.type === 'event') {
                  ...
                  if (data === '[DONE]') {
                    ...
                    // Append the received data to the assistant's message
                    assistantMessage += data

                    // Update chatbot messages with streaming content
                    setChatbots(chatbots => chatbots.map(cb =>
                      cb.id === botId
                        ? { 
                            ...cb, 
  ...
  const setLayout = (mode: LayoutMode) => {
    ...
    if (mode === 'single') {
      ...
      // Limit to 4 chatbots for quad layout
  ...
  const copyToClipboard = useCallback((botId: string) => {
    ...
    if (bot) {
      ...
      navigator.clipboard.writeText(chatContent).then(() => {
        ...
        //   description: `Chat content for ${bot.name} has been copied.`,
        //   duration: 2000,
        // })
        setTimeout(() => {
        ...
        //   description: "An error occurred while copying the chat content.",
        //   variant: "destructive",
        //   duration: 2000,
        // })
      })
  ...
  const clearConversation = useCallback((botId: string) => {
    ...
    //   description: `Chat content for ${chatbots.find(b => b.id === botId)?.name} has been cleared.`,
    //   duration: 2000,
    // })
  }, [chatbots])
  ...
  const clearAllConversations = useCallback(() => {
    ...
    //   description: "All chat contents and the prompt have been cleared.",
    //   duration: 2000,
    // })
  }, [chatbots])
  ...
  const onFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files) {
      ...
      //   description: `${files.length} file(s) have been selected for upload.`,
      //   duration: 2000,
      // })
    }
  }
  ...
  const removeFile = (fileName: string) => {
    ...
    // toast({
    //   title: "File removed",
    //   description: `${fileName} has been removed from the upload list.`,
    //   duration: 2000,
    // })
  }
  ...
  const addToScratchPad = (botId: string) => {
    ...
    if (bot) {
      ...
      //   description: `Chat content for ${bot.name} has been added to the Scratch Pad.`,
      //   duration: 2000,
      // })
    }
  }
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    setInput(value)
    if (value.endsWith('/')) {
      setInput(value.slice(0, -1)) // Remove the '/' character
                ...
                <div key={bot.id} className="bg-white rounded-lg shadow flex flex-col overflow-hidden">
                  <div className="p-4 border-b flex justify-between items-center">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <Select 
                        value={bot.name} // Change this from bot.apiName to bot.name
